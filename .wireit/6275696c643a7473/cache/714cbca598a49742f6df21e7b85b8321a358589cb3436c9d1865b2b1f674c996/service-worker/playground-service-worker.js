 ...
 */
import { ACKNOWLEDGE_SW_CONNECTION, CONNECT_SW_TO_PROJECT, MISSING_FILE_API, } from '../shared/worker-api.js';
...
const findSessionProxyClient = async (sessionId) => {
...
const getFileApi = async (sessionId) => {
    let deferred = fileAPIs.get(sessionId);
    ...
    deferred = new Deferred();
    fileAPIs.set(sessionId, deferred);
    const missingMessage = { type: MISSING_FILE_API };
    client.postMessage(missingMessage);
    return deferred.promise;
};
const getFile = async (_e, path, sessionId) => {

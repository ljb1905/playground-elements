...
export async function* processTypeScriptFiles(workerContext, results) {
    ...
    for await (const result of results) {
        if (result.kind === 'file' &&
            ...
            if (result.kind === 'file' && result.file.name === 'package.json') {
                try {
                    packageJson = JSON.parse(result.file.content);
                }
                catch (e) {
    ...
    // semantics.
    const defaultPackageJson = packageJson === undefined
        ? { type: 'module' }
        : packageJson.type === 'module'
            ? packageJson
            : { ...packageJson, type: 'module' };
    loadedFiles.set(new URL('package.json', self.origin).href, JSON.stringify(defaultPackageJson));
    ...
    // diagnostics.
    const typings = await TypesFetcher.fetchTypes(workerContext.cdn, workerContext.importMapResolver, packageJson, inputFiles.map((file) => file.file.content), workerContext.languageServiceContext.compilerOptions.lib);

...
let PlaygroundThemeDetector = class PlaygroundThemeDetector extends LitElement {
    constructor() {
        super(...arguments);
        this._filetype = 'ts';
        this._iframeSrcdoc = '';
    ...
    render() {
      ...
      <div id="palette">
        ${[...this._propertyValues].map(([prop, color]) => html `<span
              class="color"
              title=${prop}
              style="background:${color !== null && color !== void 0 ? color : 'transparent'}"
            ></span>`)}
      </div>
      ...
      <div id="buttons">
        <button
          ...
          .disabled=${![...this._propertyValues.values()].some((val) => val !== null)}
        >
          Apply
        </button>
    ...
    async firstUpdated() {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        this._playgroundWithUserText._codemirror.setOption('viewportMargin', Infinity);
    }
    async _copySample(filetype) {
    ...
    _extractStyles() {
        ...
        const fragmentToProperty = new Map();
        const ourWalker = document.createTreeWalker(this._playgroundWithUserText.shadowRoot, NodeFilter.SHOW_TEXT);
        while (ourWalker.nextNode()) {
            ...
            let property;
            const cmClasses = [...parent.classList].filter((c) => c.startsWith('cm-'));
        ...
        const theirWalker = doc.createTreeWalker(doc.body, NodeFilter.SHOW_TEXT | NodeFilter.SHOW_ELEMENT);
        let foundBackground = false;
        while (theirWalker.nextNode()) {
            const node = theirWalker.currentNode;
            if (node.nodeType === Node.TEXT_NODE) {
                const text = node.textContent.replace(/^<\/?/, '').replace(/>$/, '');
                const property = (_a = fragmentToProperty.get(text)) === null || _a === void 0 ? void 0 : _a.shift();
...
PlaygroundThemeDetector.styles = css `
    ol {
      padding-left: 24px;
    }
...
__decorate([
    state()
], PlaygroundThemeDetector.prototype, "_filetype", void 0);
__decorate([
    state()
], PlaygroundThemeDetector.prototype, "_iframeSrcdoc", void 0);
__decorate([
    state()
], PlaygroundThemeDetector.prototype, "_codeText", void 0);
__decorate([
    state()
], PlaygroundThemeDetector.prototype, "_propertyValues", void 0);
__decorate([
    query('iframe')
], PlaygroundThemeDetector.prototype, "_iframeWithUserHtml", void 0);
__decorate([
    query('playground-code-editor')
], PlaygroundThemeDetector.prototype, "_playgroundWithUserText", void 0);
PlaygroundThemeDetector = __decorate([
    customElement('playground-theme-detector')
], PlaygroundThemeDetector);
export { PlaygroundThemeDetector };
const sampleFiles = {
    ts: `

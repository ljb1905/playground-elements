...
suite('NodeModuleResolver', () => {
    const check = ({ path, packageJson, conditions = ['default', 'module', 'import'], expected, }) => {
        const resolver = new NodeModuleResolver({ conditions });
        const actual = resolver.resolve({ pkg: 'mydep', version: '1.2.3', path }, packageJson, './my-element.js');

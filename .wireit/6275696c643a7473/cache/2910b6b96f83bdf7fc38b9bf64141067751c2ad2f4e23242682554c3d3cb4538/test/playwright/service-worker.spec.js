...
test.describe('service worker', () => {
     ...
     * the given code.
     */
    const setServiceWorkerCode = (code) => {
        swCode = code;
    };
    let swCode;
    /**
     ...
     */
    const setServiceWorkerVersion = (version) => {
        swVersion = version;
    };
    let swVersion;
    /**
     ...
     */
    const setExpectedVersion = (version) => {
        expectedVersion = version;
    };
    let expectedVersion;
    const setServiceWorkerAndExpectedVersion = (version) => {
    ...
    let serviceWorkerServedDeferred = new Deferred();
    test.beforeAll(async () => {
        var _a;
        originalSwCode = await readFile('./playground-service-worker.js', 'utf8');
        const versionPattern = /version:\s*(?<quote>["'`])(?<hex>[a-z0-9]+)\k<quote>/g;
        const versionMatches = [...originalSwCode.matchAll(versionPattern)];
        if (versionMatches.length !== 1) {
            throw new Error('Expected 1 version string in playground-service-worker.js. ' +
                `Found ${versionMatches.length}.`);
        }
        const hex = (_a = versionMatches[0].groups) === null || _a === void 0 ? void 0 : _a['hex'];
        if (!hex) {
    ...
    const getIframeBodyText = async (page) => {
    ...
    const expectIframeServesVersion = async (page, version) => {
        const text = await getIframeBodyText(page);
        expect(text).toEqual(`${version} index.html`);
    };
    const clickReloadButton = async (page) => {
        await page.click('#reload-button');
    };
    const expectConsoleLog = (page, expect) => new Promise((resolve) => page.on('console', async (msg) => {
    ...
    test('recover from outdated service worker', async ({ browser }) => {
        ...
        const loggedAboutUpdate = expectConsoleLog(page, 'Playground service worker is outdated. Want new but got old. Waiting for update.');
        await page.goto(indexUrl);

...
import { InvalidModuleSpecifierError, InvalidPackageConfigError, InvalidPackageTargetError, PackagePathNotExportedError, } from './errors.js';
function emitFolderMapDeprecation(match, pjsonUrl, isExports, base) {
    const pjsonPath = fileURLToPath(pjsonUrl);
    console.warn(`Use of deprecated folder mapping "${match}" in the ${isExports ? '"exports"' : '"imports"'} field module resolution of the package at ${pjsonPath}${base ? ` imported from ${base}` : ''}.\n` +
        `Update this package.json to use a subpath pattern like "${match}*".`, 'DeprecationWarning', 'DEP0148');
}
function makeExportsNotFoundError(subpath, packageJSONUrl, base) {
    return new PackagePathNotExportedError(fileURLToPath(new URL('.', packageJSONUrl)), subpath, base);
}
function makeInvalidSubpathError(subpath, packageJSONUrl, internal, base) {
...
function makeInvalidPackageTargetError(subpath, target, packageJSONUrl, internal, base) {
    ...
    return new InvalidPackageTargetError(packageJSONUrl, subpath, target, internal, base);
}
const invalidSegmentRegEx = /(^|\\|\/)(\.\.?|node_modules)(\\|\/|$)/;
const patternRegEx = /\*/g;
function resolvePackageTargetString(target, subpath, match, packageJSONUrl, base, pattern, internal, 
// eslint-disable-next-line @typescript-eslint/no-unused-vars
_conditions) {
    if (subpath !== '' && !pattern && target[target.length - 1] !== '/') {
        throw makeInvalidPackageTargetError(match, target, packageJSONUrl, internal, base);
    }
    ...
    if (invalidSegmentRegEx.test(subpath)) {
        throw makeInvalidSubpathError(match + subpath, packageJSONUrl, internal, base);
    }
    if (pattern) {
...
function resolvePackageTarget(packageJSONUrl, target, subpath, packageSubpath, base, pattern, internal, conditions) {
    if (typeof target === 'string') {
        ...
        for (let i = 0; i < keys.length; i++) {
            const key = keys[i];
            if (isArrayIndex(key)) {
                throw new InvalidPackageConfigError(fileURLToPath(packageJSONUrl), base, '"exports" cannot contain numeric property keys.');
            }
        }
        for (let i = 0; i < keys.length; i++) {
            const key = keys[i];
    ...
    throw makeInvalidPackageTargetError(packageSubpath, target, packageJSONUrl, internal, base);
}
...
_exports, packageJSONUrl, base) {
    ...
    for (let j = 0; j < keys.length; j++) {
        ...
        if (i++ === 0) {
            ...
            throw new InvalidPackageConfigError(fileURLToPath(packageJSONUrl), base, '"exports" cannot contain some keys starting with \'.\' and some not.' +
...
export function packageExportsResolve(packageJSONUrl, packageSubpath, packageConfig, base, conditions) {
    let _exports = packageConfig.exports;
    if (isConditionalExportsMainSugar(_exports, packageJSONUrl, base)) {
        _exports = { '.': _exports };
    }
    _exports = _exports;
    if (Object.prototype.hasOwnProperty.call(_exports, packageSubpath) &&
    ...
    let bestMatch = '';
    let bestMatchSubpath = undefined;
    ...
    if (bestMatch) {
        ...
        bestMatchSubpath, bestMatch, base, pattern, false, conditions);
        if (resolved === null || resolved === undefined) {
            throw makeExportsNotFoundError(packageSubpath, packageJSONUrl, base);
        }
        if (!pattern) {

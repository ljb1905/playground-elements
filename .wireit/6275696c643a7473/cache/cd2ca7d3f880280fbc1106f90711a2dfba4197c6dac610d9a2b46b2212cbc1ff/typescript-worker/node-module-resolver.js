...
export class NodeModuleResolver {
     ...
     */
    resolve(location, packageJson, base) {
        ...
        const resolved = packageExportsResolve(packageJsonUrl, packageSubpath, packageJson, base, this._conditions);
        if (!resolved.pathname.startsWith(packageBase)) {
            throw new Error(`Unexpected error: ${resolved.pathname} expected to start with ${packageBase}`);
        }
 ...
 * Convert e.g. "." to "" and "./foo.js" to "foo.js".
 */
const removeRelativePrefix = (path) => {
    if (path === '.') {
        return '';
    }
    if (path.startsWith('./')) {

...
import { parseNpmStyleSpecifier, changeFileExtension, classifySpecifier, resolveUrlPath, trimTrailingSlash, trimLeadingSlash, } from './util.js';
import { NodeModulesLayoutMaker, } from './node-modules-layout-maker.js';
/**
...
export class TypesFetcher {
    constructor(cdn, importMapResolver, rootPackageJson) {
        this._rootDependencies = {};
        this._dependencyGraph = {};
        this._filesByPackageVersion = new Map();
        this._cdn = cdn;
     ...
     * "es2020", "DOM".
     */
    static async fetchTypes(cdn, importMapResolver, rootPackageJson, sources, tsLibs) {
        ...
        await Promise.allSettled([
            ...sources.map((source) => fetcher._handleBareAndRelativeSpecifiers(source, root)),
            ...tsLibs.map((lib) => fetcher._addTypeScriptStandardLib(lib)),
        ]);
        const layout = new NodeModulesLayoutMaker().layout(fetcher._rootDependencies, fetcher._dependencyGraph);
        const files = new Map();
    ...
    async _addTypeScriptStandardLib(lib) {
        return this._handleBareSpecifier(`typescript/lib/lib.${lib.toLowerCase()}.js`, root);
    }
    async _handleBareAndRelativeSpecifiers(sourceText, referrer) {
    ...
    async _handleBareSpecifier(bare, referrer) {
        var _a, _b;
        ...
        if (!handledByImportMap) {
            location.version = await this._getDependencyVersion(referrer, location.pkg);
        }
        ...
        if (((_b = (_a = this._filesByPackageVersion
            .get(location.pkg)) === null || _a === void 0 ? void 0 : _a.get(location.version)) === null || _b === void 0 ? void 0 : _b.get(location.path)) !== undefined) {
            return;
        }
    ...
    async _handleRelativeSpecifier(relative, referrer) {
        var _a, _b;
        const location = {
    ...
    async _getDependencyVersion(from, to) {
        var _a, _b;
        const packageJson = from === root
            ? this._rootPackageJson
            : await this._fetchPackageJsonAndAddToOutputFiles(from);
        return (_b = (_a = packageJson === null || packageJson === void 0 ? void 0 : packageJson.dependencies) === null || _a === void 0 ? void 0 : _a[to]) !== null && _b !== void 0 ? _b : 'latest';
    }
    async _getDtsPath(location) {
        var _a, _b, _c;
        if (location.path !== '') {
            return changeFileExtension(location.path, 'd.ts');
        }
        const packageJson = await this._fetchPackageJsonAndAddToOutputFiles(location);
        return ((_c = (_b = (_a = packageJson === null || packageJson === void 0 ? void 0 : packageJson.typings) !== null && _a !== void 0 ? _a : packageJson === null || packageJson === void 0 ? void 0 : packageJson.types) !== null && _b !== void 0 ? _b : ((packageJson === null || packageJson === void 0 ? void 0 : packageJson.main) !== undefined
            ? changeFileExtension(packageJson.main, 'd.ts')
            : undefined)) !== null && _c !== void 0 ? _c : 'index.d.ts');
    }
    async _fetchPackageJsonAndAddToOutputFiles(location) {
        const result = await this._fetchAndAddToOutputFiles({
            ...location,
            path: 'package.json',
        });
        if (result.error !== undefined) {
            throw new Error(`Could not fetch package.json for ` +
                `${location.pkg}@${location.version}: ${result.error}`);
        }
        return JSON.parse(result.result);
    }
    async _fetchAndAddToOutputFiles(location) {
        const importMapUrl = this._importMapResolver.resolve(trimTrailingSlash(`${location.pkg}/${location.path}`));
     ...
     */
    _addEdgeToDependencyGraph(from, to) {
        if (from === root) {
     ...
     */
    async _materializeNodeModulesTree(layout, fileMap, prefix) {
        var _a;
        for (const [pkg, entry] of Object.entries(layout)) {
            const files = (_a = this._filesByPackageVersion.get(pkg)) === null || _a === void 0 ? void 0 : _a.get(entry.version);
            if (files === undefined) {
                continue;
            }
            ...
            await this._materializeNodeModulesTree(entry.nodeModules, fileMap, `${prefix}${pkg}/node_modules/`);
        }
    }
}

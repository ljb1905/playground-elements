...
suite('playground-code-editor', () => {
    ...
    test('is registered', () => {
        assert.instanceOf(document.createElement('playground-code-editor'), PlaygroundCodeEditor);
    });
    ...
    suite('history', () => {
        ...
        test(`and doc instance cache won't drive editor value`, async () => {
            ...
            await raf();
            assert.equal(editorInternals._codemirror.getValue(), 'value with no cache');
        });
        ...
        test('is updated if value gets changed with doc cache', async () => {
            ...
            await raf();
            assert.equal(editorInternals._codemirror.getValue(), 'override document key 1');
            (_a = editorInternals._codemirror) === null || _a === void 0 ? void 0 : _a.undo();
            await raf();
            assert.equal(editorInternals._codemirror.getValue(), 'document key 1');
        });
    ...
    suite('syntax highlight', () => {
        ...
        const findNodeWithText = (node, text) => {
            var _a;
            if (((_a = node.textContent) === null || _a === void 0 ? void 0 : _a.trim()) === text) {
                return node;
            }
        ...
        const assertHighlight = async (type, value, text, color) => {
            const editor = document.createElement('playground-code-editor');
            editor.type = type;
            editor.value = value;
            container.appendChild(editor);
        ...
        test('html', async () => assertHighlight('html', '<p>foo</p>', '<p>', tagColor));
        test('css', async () => assertHighlight('css', 'p { color: blue; }', 'blue', keywordColor));
        test('js', async () => assertHighlight('js', 'if (true) {}', 'true', atomColor));
        test('ts', async () => assertHighlight('ts', 'const x: string;', 'string', typeColor));
        test('jsx', async () => assertHighlight('jsx', 'const foo = () => <p>foo</p>;', 'p', tagColor));
        test('tsx', async () => assertHighlight('tsx', 'const x: () => unknown = () => <p>foo</p>;', 'p', tagColor));
        test('html-in-js', async () => assertHighlight('js', 'html`<p>foo</p>`', '<p>', tagColor));
        test('html-in-ts', async () => assertHighlight('ts', 'html`<p>foo</p>`', '<p>', tagColor));
        test('css-in-js', async () => assertHighlight('js', 'css`p { color: blue; }`', 'blue', keywordColor));
        test('css-in-ts', async () => assertHighlight('ts', 'css`p { color: blue; }`', 'blue', keywordColor));
        test('json', async () => assertHighlight('json', '{"foo": 123}', '"foo"', stringColor));
    });
    suite('comment toggle', () => {
        async function assertToggle(type, value, expect) {
            const editor = document.createElement('playground-code-editor');
            editor.type = type;
            editor.value = value;
            container.appendChild(editor);
        ...
        test('ts', async () => assertToggle('ts', 'const g = 3;', '// const g = 3;'));
        test('js', async () => assertToggle('js', 'const g = 3;', '// const g = 3;'));
        test('html', async () => assertToggle('html', '<p>foo</p>', '<!-- <p>foo</p> -->'));
        test('css', async () => assertToggle('css', 'p { color: blue; }', '/* p { color: blue; } */'));
        test('ignored when readonly', async () => {
            ...
            editor.shadowRoot.querySelector('div').innerText, 'const g = 3;');
        });
    });
});

 ...
 */
import { CONNECT_SW_TO_PROJECT, CONNECT_PROJECT_TO_SW, CONFIGURE_PROXY, MISSING_FILE_API, UPDATE_SERVICE_WORKER, } from './shared/worker-api.js';
...
(async () => {
    try {
        ...
        parent.window.console.warn('Playground sandbox is executing with the same origin as its parent.', 'This is a security risk.', 'https://github.com/google/playground-elements#sandbox-security');
    ...
    const { scope, port: parentPort } = await new Promise((resolve) => {
    ...
    const registration = await navigator.serviceWorker.register(new URL('playground-service-worker.js', import.meta.url).href, { scope });
    /** https://www.w3.org/TR/service-workers/#get-newest-worker-algorithm */
    const getNewestWorker = () => { var _a, _b; return (_b = (_a = registration.installing) !== null && _a !== void 0 ? _a : registration.waiting) !== null && _b !== void 0 ? _b : registration.active; };
    /**
     * Resolve when the given service worker reaches its activated state.
     */
    const activated = async (sw) => {

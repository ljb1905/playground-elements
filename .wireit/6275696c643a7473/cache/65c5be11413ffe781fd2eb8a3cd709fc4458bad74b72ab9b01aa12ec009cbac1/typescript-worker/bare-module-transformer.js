...
import { MergedAsyncIterables, parseNpmStyleSpecifier, resolveUrlPath, charToLineAndChar, fileExtension, classifySpecifier, relativeUrlPath, } from './util.js';
...
export class BareModuleTransformer {
    constructor(cdn, importMapResolver) {
        this._emittedExternalDependencies = new Set();
        this._nodeResolver = new NodeModuleResolver({
    ...
    async *process(results) {
        ...
        const output = new MergedAsyncIterables();
        ...
        for await (const result of results) {
            if (result.kind === 'file' && result.file.name.endsWith('.js')) {
                ...
                if (result.kind === 'file' && result.file.name === 'package.json') {
                    let parsed;
                    try {
                        parsed = JSON.parse(result.file.content);
                    }
                    catch (e) {
     ...
     * those specifiers.
     */
    async *_handleModule(file, getPackageJson, output) {
        ...
        for (let i = specifiers.length - 1; i >= 0; i--) {
            ...
            transforms.push({
                info: specifiers[i],
                newSpecifierPromise: this._handleSpecifier(oldSpecifier, file.file.name, getPackageJson, output),
            });
        }
        for (const { info: { s: start, e: end, n: oldSpecifier, d: dynamicStart }, newSpecifierPromise, } of transforms) {
            let newSpecifier;
            try {
     ...
     */
    async _handleSpecifier(specifier, referrer, getPackageJson, output) {
        ...
        if (kind === 'bare') {
            return this._handleBareSpecifier(specifier, referrer, getPackageJson, output);
        }
        ...
        if (!fileExtension(specifier)) {
            ...
            return this._handleBareSpecifier(bare, referrer, 
     ...
     */
    async _handleBareSpecifier(specifier, referrer, getPackageJson, output) {
        var _a, _b, _c;
        ...
        if (!location.version) {
            location.version =
                (_c = (_b = (_a = (await getPackageJson())) === null || _a === void 0 ? void 0 : _a.dependencies) === null || _b === void 0 ? void 0 : _b[location.pkg]) !== null && _c !== void 0 ? _c : 'latest';
        }
     ...
     */
    async *_fetchExternalDependency(location, output) {
 ...
 */
const makeJsonParseDiagnostic = (e, file) => {
    var _a;
    const start = (_a = extractPositionFromJsonParseError(e.message, file.content)) !== null && _a !== void 0 ? _a : {
        line: 0,
        character: 0,
    };
    return {
 ...
 */
const extractPositionFromJsonParseError = (message, json) => {
